...
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MAX_LEVEL;
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MODE_BURNING;
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MODE_EMPTY;
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MODE_INFUSING;
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MODE_PRIMED_WATER;
import static grondag.doomtree.block.AlchemicalBasinBlockEntity.MODE_PRIMED_WITCHWATER;
...
public class AlchemicalBasinBlock extends BlockWithEntity {
	...
	public boolean activate(BlockState blockState, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (mode == MODE_EMPTY) {
			if (item == DoomItems.ICHOR_BUCKET || item == Items.WATER_BUCKET) {
				if (!world.isClient) {
		...
		final int doomFuelValue = item == DoomItems.DOOM_FRAGMENT_ITEM || item  == DoomItems.DOOM_LEAF_ITEM ? 1
				: item == DoomItems.DOOM_LOG_ITEM || item  == DoomItems.PLACED_DOOM_LOG_ITEM ? 4 : 0;
		...
		final int wardingFuelValue = item == DoomItems.WARDING_ESSENCE_ITEM ? 1
				: item == DoomItems.WARDING_ESSENCE_BLOCK_ITEM ? 4 : 0;
