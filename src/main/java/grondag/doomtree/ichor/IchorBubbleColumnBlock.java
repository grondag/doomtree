...
public class IchorBubbleColumnBlock extends Block implements FluidDrainable {
	...
	public IchorBubbleColumnBlock(Settings settings) {
		super(settings);
	...
	public static void update(IWorld world, BlockPos pos, boolean bool) {
		if (isStillWater(world, pos)) {
			world.setBlockState(pos, DoomBlocks.ICHOR_BUBBLE_COLUMN.getDefaultState().with(DRAG, bool), 2);
		}
	}
	
	public static boolean isStillWater(IWorld world, BlockPos pos) {
		FluidState state = world.getFluidState(pos);
		return world.getBlockState(pos).getBlock() == DoomBlocks.ICHOR_BLOCK && state.getLevel() >= 8 && state.isStill();
	}
	
	private static boolean calculateDrag(BlockView view, BlockPos pos) {
		BlockState state = view.getBlockState(pos);
		Block block = state.getBlock();
		if (block == DoomBlocks.ICHOR_BUBBLE_COLUMN) {
	...
	public FluidState getFluidState(BlockState state) {
		return DoomFluids.ICHOR.getStill(false);
	}
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction dir, BlockState state2, IWorld world, BlockPos pos, BlockPos pos2) {
		if (!state.canPlaceAt(world, pos)) {
			...
			if (dir == Direction.DOWN) {
				world.setBlockState(pos, DoomBlocks.ICHOR_BUBBLE_COLUMN.getDefaultState().with(DRAG, calculateDrag(world, pos2)), 2);
			} else if (dir == Direction.UP && state2.getBlock() != DoomBlocks.ICHOR_BUBBLE_COLUMN && isStillWater(world, pos2)) {
				world.getBlockTickScheduler().schedule(pos, this, this.getTickRate(world));
			}
			world.getFluidTickScheduler().schedule(pos, DoomFluids.ICHOR, DoomFluids.ICHOR.getTickRate(world));
	...
	public boolean canPlaceAt(BlockState state, ViewableWorld world, BlockPos pos) {
		...
		return block == DoomBlocks.ICHOR_BUBBLE_COLUMN;
	}
