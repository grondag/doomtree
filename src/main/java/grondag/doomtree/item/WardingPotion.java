...
public class WardingPotion extends Item {
	...
	public ItemStack finishUsing(final ItemStack stack, final World world, final LivingEntity entity) {
		...
		final StatusEffectInstance warding = entity.getStatusEffect(DoomEntities.WARDING_EFFECT);

		if (warding == null) {
			if (!world.isClient) {
				entity.addPotionEffect(new StatusEffectInstance(DoomEntities.WARDING_EFFECT, WardingEffect.TICKS_PER_LEVEL, 0, false, false, true));
			}
		} else if (warding.getDuration() <= WardingEffect.MAX_REFILL_TICKS) {
			StatusEffectAccess.access(warding).fermion_addDuration(WardingEffect.TICKS_PER_LEVEL);
		}
